...
| Mathematical formula | `math` or inline `\(...\)` in text |
...
| A formula or derivation | `math` for display, `\(...\)` for inline | Formula is TeX, with mhchem `\ce{...}` / `\pu{...}` for chemistry and units; never use `$...$`. |
